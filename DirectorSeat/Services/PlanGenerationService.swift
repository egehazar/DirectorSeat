...
class PlanGenerationService {
    ...
    static let coverageInstructions = """
        ...
        - Because it punches into ONE wide, that wide MUST be framed for it. The covered shot's shot_type MUST be "wide" (never "medium" or "close-up"), and its direction_text / camera_placement / actor_direction MUST instruct a TRUE WIDE TWO-SHOT: both people fully in frame with headroom and clear space around each, placed close (e.g. seated across a small table, angled slightly toward each other) so the LEFT person occupies a croppable left portion of the frame and the RIGHT person a croppable right portion. Static, locked phone for the entire take. A medium or tight shot makes crop-zoom impossible — a genuine wide two-shot is mandatory on a covered shot. (The audience still gets the intimacy: the engine delivers the close-ups by punching in; the wide is only the source the cuts are carved from.)
